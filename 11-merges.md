...
Mercurial detects that our changes have diverged with the changes in the remote repo on Bitbucket and stops us from creating a confusing situation on there.
...
`hg pull` tells us there's now an extra head in our repo and suggests that we use `hg heads` to inspect the situation and `hg merge` to resolve it.
...
The same commits may have different revision numbers in different clones. These revision numbers are assigned in the order that a clone acquires new commits.
On the other hand,
the identifiers
...
This tells us thar our working directory is in a merged state.
